    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            ...
            if str(exc_val) == 'no such table: expenses':
                ...
                print('If you see this error please first run init command')  
        self.cursor.close()
        self.connection.close()

#build the databse with expenses table
# we can change it later to take table name so we can have multiple table
def init():
    'initial the database'
    with SqlConnection('data.db') as c:
